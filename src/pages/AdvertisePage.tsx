...
export default function AdvertisePage() {
  ...
  return (
    <div className="min-h-screen bg-cream">
      {/* Hero */}
      <div className="bg-gradient-to-br from-forest to-forest-light text-white py-16 px-4">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 -mt-8">
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={title} className="bg-white rounded-2xl shadow-sm border border-sage-dark/20 p-5">
              <Icon className="w-6 h-6 text-forest mb-2" />
              <h3 className="text-sm font-bold text-earth mb-1">{title}</h3>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        <h2 className="text-2xl font-bold text-earth text-center mb-8">Simple pricing</h2>
        <div className="grid sm:grid-cols-3 gap-4">
          {TIERS.map(tier => (
            <div
              key={tier.name}
              className={`bg-white rounded-2xl shadow-sm border p-6 ${
                tier.popular ? 'border-forest ring-2 ring-forest/20' : 'border-sage-dark/20'
              }`}
            >
              {tier.popular && (
              ...
              <h3 className="text-lg font-bold text-earth">{tier.name}</h3>
              <p className="text-2xl font-bold text-forest mt-1">{tier.price}</p>
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6 pb-16">
        <div className="bg-white rounded-2xl shadow-sm border border-sage-dark/20 p-8">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-xl font-bold text-earth">Get started</h2>
          </div>
          ...
          {status === 'sent' ? (
            <div className="text-center py-8">
              ...
              <p className="text-lg font-semibold text-earth">Thanks! We'll be in touch shortly.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input required value={form.business_name} onChange={e => setForm(p => ({ ...p, business_name: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-sage-dark/40 text-sm focus:outline-none focus:ring-2 focus:ring-forest/30" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-earth mb-1">Your Name *</label>
                  <input required value={form.contact_name} onChange={e => setForm(p => ({ ...p, contact_name: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-sage-dark/40 text-sm focus:outline-none focus:ring-2 focus:ring-forest/30" />
                </div>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-earth mb-1">Email *</label>
                  <input type="email" required value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-sage-dark/40 text-sm focus:outline-none focus:ring-2 focus:ring-forest/30" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-earth mb-1">Phone</label>
                  <input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-sage-dark/40 text-sm focus:outline-none focus:ring-2 focus:ring-forest/30" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-earth mb-1">Preferred Tier</label>
                <select value={form.tier} onChange={e => setForm(p => ({ ...p, tier: e.target.value }))} className="w-full px-3 py-2.5 rounded-xl border border-sage-dark/40 text-sm focus:outline-none focus:ring-2 focus:ring-forest/30 bg-white">
              ...
              <div>
                ...
                <textarea value={form.message} onChange={e => setForm(p => ({ ...p, message: e.target.value }))} rows={3} className="w-full px-3 py-2.5 rounded-xl border border-sage-dark/40 text-sm focus:outline-none focus:ring-2 focus:ring-forest/30 resize-none" placeholder="Tell us about your business or any questions" />
              </div>
              <button type="submit" disabled={status === 'sending'} className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-forest text-white rounded-xl text-base font-semibold hover:bg-forest-light transition-colors disabled:opacity-50">
